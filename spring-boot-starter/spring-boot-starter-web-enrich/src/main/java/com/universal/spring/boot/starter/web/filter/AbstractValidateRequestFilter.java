...
import com.universal.util.VersionUtils;
    ...
    private void validateBundle(final ServletRequest request) throws ValidationException {
        ...
        if (bundles.containsKey(bundle)) {
            ...
            if (!version.matches(versionDeviceOS.getKey())) {
                throw new ValidationException(Code.SYS_INVALID_BUNDLE, String.format("BUNDLE %s 与VERSION %s 不匹配", bundle, version));
            }
            ...
            if (StringUtils.isNotBlank(deviceOS)) {
                if (!deviceOS.matches(versionDeviceOS.getValue())) {
                    throw new ValidationException(Code.SYS_INVALID_BUNDLE, String.format("BUNDLE %s 与OS %s 不匹配", bundle, deviceOS));
                }
            }
        } else {
            throw new ValidationException(Code.SYS_INVALID_BUNDLE, String.format("非法的BUNDLE %s", bundle));
        }
    }
